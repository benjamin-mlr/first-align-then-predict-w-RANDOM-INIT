...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        ...
        torch_dtype = kwargs.pop("torch_dtype", None)
        random_init_layers = kwargs.pop("random_init_layers", [])


        from_pt = not (from_tf | from_flax)

        if len(random_init_layers) > 0:
            assert from_pt, f"RANDOM-INIT only supported if loading from pytorch"
        ...
        if from_tf:
            ...
            model, missing_keys, unexpected_keys, mismatched_keys, error_msgs = cls._load_state_dict_into_model(
                ...
                random_init_layers=random_init_layers,
            )
    ...
    def _load_state_dict_into_model(
        cls, model, state_dict, pretrained_model_name_or_path, ignore_mismatched_sizes=False, _fast_init=True, random_init_layers=None,
    ):

        # Convert old format to new format if needed from a PyTorch state_dict
        if random_init_layers is None:
            random_init_layers = []
        old_keys = []
        new_keys = []
        ...
        def load(module, prefix='', info_layer_random_init=None):
            ...
            if is_deepspeed_zero3_enabled():
                import deepspeed
                if len(random_init_layers) > 0:
                    print(f"Warning: RANDOM-INIT is activated with deepspeed but has not been tested")
            ...
            for name, child in module._modules.items():
                ...
                matching_not_load = []
                # RANDOM-INIT: checking in random_init_layers if it matches the current layer $name
                for pattern in random_init_layers:
                    matching = re.match(pattern, prefix + name)
                    if matching is not None:
                        # if it matches we add the layer name in the list matching_not_load
                        matching_not_load.append(matching)

                if child is not None and len(matching_not_load) == 0:
                    info_layer_random_init = load(child, prefix + name + '.', info_layer_random_init)
                else:
                    # if at least one layer has been matched to an element of the random_init_layers we do not load it (i.e. we RANDOM-INIT it)
                    if info_layer_random_init is not None:
                        info_layer_random_init.append(prefix + name + '.')
            return info_layer_random_init

        def load_old(module: nn.Module, prefix=""):
            local_metadata = {} if metadata is None else metadata.get(prefix[:-1], {})
            args = (state_dict, prefix, local_metadata, True, [], [], error_msgs)
            if is_deepspeed_zero3_enabled():
                import deepspeed
            ...
            for name, child in module._modules.items():
                if child is not None:
                    print("MODEL original loading : child {} full {} ".format(name, prefix + name + '.'))
                    load(child, prefix + name + ".")
                    #print(f"Loading : {prefix + name + '.'}")
        ...
        info_layer_random_init = []
        info_layer_random_init = load(model_to_load, prefix=start_prefix, info_layer_random_init=info_layer_random_init)

        if len(info_layer_random_init) > 0:
            logger.warning(
                f"RANDOM-INIT was applied to the following layers {info_layer_random_init} based on argument {random_init_layers}"
            )
